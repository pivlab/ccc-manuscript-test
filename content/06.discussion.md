...
In particular, CCC alone detected gene pairs with complex nonlinear patterns from the sex chromosomes, highlighting the role that not-only-linear coefficients can play in capturing sex-specific differences.
...
This example, in particular, highlights the importance of considering sex as a biological variable (SABV) to avoid overlooking important differences between men and women, for instance, in disease manifestations.
...
The single parameter of CCC, $k_{\mathrm{max}}$, controls the maximum complexity of patterns found and also impacts the computation time.
...
Not-only-linear coefficients can also disentangle intricate yet relevant patterns from gene expression data alone that were replicated in models integrating different data modalities.
