...
We next performed a systematic evaluation using the top 100 discrepant gene pairs between the correlation coefficient, nonlinear relationships, and gene expression.
...
For this, we used the top five most commonly autodetected cell types for each coefficient and assessed connectivity in the resulting networks.
The top 5 predicted cell types for gene pairs highly ranked by the correlation coefficient and not by the rest were all blood-specific, including macrophage, leukocyte, natural killer cell, blood, and mononuclear phagocyte.
The average probability of interaction between genes in these correlation coefficient-ranked networks was significantly higher than the other coefficients, with all medians larger than 67% and first quartiles above 41% across predicted cell types.
...
The interaction probabilities in these Pearson-ranked networks were also generally lower than in the correlation coefficient, except for blood-specific gene pairs.
The associations exclusively detected by the correlation coefficient in whole blood from GTEx were more strongly replicated in these independent networks that incorporated multiple data modalities.
Correlation coefficient-ranked gene pairs not only had high probabilities of belonging to the same biological process but were also predicted to be specifically expressed in blood cell lineages.
...
This lack of replication in GIANT suggests that top Pearson-ranked gene pairs in GTEx might be driven mainly by outliers, which is consistent with our earlier observations of outlier-driven associations.
